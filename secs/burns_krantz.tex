...
\begin{proof}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=3.0cm,y=3.0cm]
      ...
      \begin{scriptsize}
        ...
        \draw[color=black] (0.45,0.33) node {$z$};
        \draw[color=black] (0.18,0.14) node {$C(z)$};
        \draw[color=black] (0.62,0.365) node {$r(z)$};
        \fill [color=black] (1,0) circle (1.5pt);
        \draw[color=black] (1.04,0.05) node {$1$};
        \fill [color=black] (0.27,0.54) circle (1.5pt);
        \draw[color=black] (0.21,0.58) node {$A$};
        \fill [color=black] (1,0) circle (1.5pt);
  ...
  \marginpar{Qualche spiegazione in più (sfruttando il disegno)?}
  \begin{align*}
