...
Com'è noto, l'olomorfia per funzioni a valori complessi è molto più forte della derivabilità in senso reale (in particolare, è equivalente all'analiticità), dunque è una condizione piuttosto rigida. Fra i vari risultati noti per tali funzioni, ci interessa il lemma di Schwarz-Pick: vogliamo vederlo appunto come risultato di rigidità, per poi dimostrarne una versione al bordo.
...
\begin{oss} \label{dom}
  Ogni automorfismo è olomorfo in $\mathbb{C}\setminus\{1/\bar{a}\}$, o tutto $\mathbb{C}$ se $a=0$. Dalla dimostrazione abbiamo anche che $f(\partial \mathbb{D}) \subset \partial \mathbb{D}$; inoltre, per $|z|>1$ con $z \not=1/\bar{a}$ si ha $|f(z)|>1$.
\end{oss}
...
\begin{proof}
  ...
  Abbiamo poi $|(\gamma_2 \circ f \circ \gamma_1)'(0)| \le 1$, da cui $\left|\gamma_2'\bigl(f(w)\bigr)f'(w)\gamma_1'(0)\right| \le 1$. Valgono le seguenti uguaglianze:
  \begin{gather*}
...
\begin{defn}
   Sia $[z,w]:=f_{w,0}(z)=\dfrac{z-w}{1-\bar{w}z}$; chiamiamo \textit{distanza pseudo-iperbolica} la quantità $p(z,w):=|[z,w]|$. Vedremo tra poco che, come il nome suggerisce, è una distanza.
\end{defn}
...
\begin{proof}
  ...
  L'unica cosa non ovvia da dimostrare è la disuguaglianza triangolare. Applicando la tangente iperbolica a entrambi i membri e sfruttando l'uguaglianza $\text{tanh}\,(a+b)=\frac{\text{tanh}\,(a)+\text{tanh}\,(b)}{1+\text{tanh}\,(a)\,\text{tanh}\,(b)}$, per stretta crescenza di $\text{tanh}$ otteniamo che la disuguaglianza da dimostrare è equivalente a
  \begin{align*}
  ...
  che è quello che otteniamo inserendo $z_0=0$ nella disuguaglianza \eqref{star} e usando che $p(0,z)=p(z,0)=|z|$.
\end{proof}
...
\begin{oss}
  Poiché $p$ è a valori non negativi, dalla disuguaglianza \eqref{star} segue che anch'essa soddisfa la disuguaglianza triangolare; dunque è, come già anticipato, una distanza. È chiamata anche ``distanza di Möbius''.
\end{oss}
...
\begin{oss} \label{geoingeo}
  Poiché $p$ è invariante sotto l'azione di $\text{Aut}(\mathbb{D})$, anche $\omega$ lo è, dunque gli automorfismi mandano geodetiche in geodetiche; inoltre, conservano l'ordine di appartenenza dei punti.
\end{oss}

\begin{ex} \label{diam}
  Dato $z_0 \in \mathbb{D}\setminus\{0\}$, la curva $\sigma(t)=\tanh(t)\frac{z_0}{|z_0|}$ (il diametro passante per $z_0$) è una geodetica. Che sia il diametro indicato segue dal fatto che la funzione $\tanh:\mathbb{R} \longrightarrow (-1,1)$ è biettiva. Che sia una geodetica segue dalla definizione di $\omega$, dalla formula già citata per $\tanh(a+b)$ e dal fatto che $\tanh$ è dispari.
\end{ex}
...
\begin{proof}
  ...
  Supponiamo ora che valga $\omega(z_1,z_3)=\omega(z_1,z_2)+\omega(z_2,z_3)$. Per transitività di $\text{Aut}(\mathbb{D})$ e invarianza di $\omega$, possiamo supporre $z_2=0$. Usando la definizione di $\omega$, l'uguaglianza si riscrive come
